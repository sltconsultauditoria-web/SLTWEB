...
const Guias = () => {
  ...
  useEffect(() => {
    const carregarDados = async () => {
      try {
        ...
        setGuiasGeradas(guiasData.map((item) => ({
          id: item.id,
          empresa: item.empresa || item.data?.empresa || '',
          tipo: item.tipo || item.data?.tipo || 'DAS',
          regime: item.regime || item.data?.regime || '',
          competencia: item.competencia || item.data?.competencia || '',
          valor: Number(item.valor || item.data?.valor || 0),
          vencimento: item.vencimento || item.data?.vencimento || '',
          status: item.status || item.data?.status || 'pendente',
        })));
      } catch (error) {
  ...
  const calcularDAS = () => {
    ...
    if (isMEI) {
      // Cálculo MEI
      const atividade = empresa?.atividade || 'servicos';
      ...
      const receitaBruta = Number(empresa?.receita_bruta || 0);
      const anexo = empresa?.anexo || 'I';
      const tabela = tabelasSimples[anexo];
      const fatorR = Number(empresa?.fator_r || 0);
  ...
  const recalcularDAS = () => {
    ...
    // Recalculo local com base nos dados informados
    setTimeout(() => {
