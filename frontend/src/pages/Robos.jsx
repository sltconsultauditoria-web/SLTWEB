...
const Robos = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statusRes, spStatusRes, historyRes, filesRes] = await Promise.all([
        api.get('/robots/ingestion/status'),
        api.get('/sharepoint/status'),
        api.get('/robots/ingestion/history?limit=10'),
        api.get('/robots/ingestion/files?limit=20')
      ]);
      
      setStatus(statusRes.data);
      setSharepointStatus(spStatusRes.data);
      setHistory(Array.isArray(historyRes.data) ? historyRes.data : historyRes.data.jobs || []);
      setFiles(Array.isArray(filesRes.data) ? filesRes.data : filesRes.data.files || []);
    } catch (error) {
  ...
  const handleStartScheduler = async () => {
    ...
    try {
      await api.post('/robots/ingestion/start');
      await loadData();
    } catch (error) {
  ...
  const handleStopScheduler = async () => {
    ...
    try {
      await api.post('/robots/ingestion/stop');
      await loadData();
    } catch (error) {
  ...
  const handleRunNow = async () => {
    ...
    try {
      await api.post('/robots/ingestion/run-now');
