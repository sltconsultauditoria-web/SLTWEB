...
const Certidoes = () => {
    ...
    const carregarCertidoes = async () => {
      try {
        ...
        setCertidoes(items.map((item) => ({
          ...
          cnpj: item.cnpj || item.data?.cnpj || '',
          tipo: item.tipo || item.data?.tipo || 'Certidao',
          dataEmissao: item.dataEmissao || item.data_emissao || item.data?.dataEmissao || item.data?.data_emissao || '',
          dataValidade: item.dataValidade || item.data_validade || item.data?.dataValidade || item.data?.data_validade || '',
          situacao: item.situacao || item.data?.situacao || 'regular',
          arquivo: item.arquivo || item.data?.arquivo || null,
        })));
      } catch (error) {
        console.error('Erro ao carregar certidoes:', error);
        setCertidoes([]);
      }
    };
    carregarCertidoes();
  ...
  const empresas = Array.from(new Set(certidoes.map((c) => c.empresa).filter(Boolean)));
                ...
                {filteredCertidoes.map((cert) => {
                  ...
                  return (
                    <tr key={cert.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">{formatarData(cert.dataEmissao)}</td>
                      <td className="px-4 py-3 text-sm">{formatarData(cert.dataValidade)}</td>
                      <td className="px-4 py-3">
