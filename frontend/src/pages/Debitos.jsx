  ...
  const consultarDebitos = async () => {
    ...
    try {
      ...
      const filtrados = items.filter((item) => {
        ...
        return !empresaId || empresaId === selectedEmpresa;
      });
      const agrupado = { situacao: filtrados.length ? 'pendencias' : 'regular', federal: [], estadual: [], municipal: [], fgts: [], dividaAtiva: [] };
      filtrados.forEach((item) => {
        const data = item.data || item;
        const grupo = data.grupo || data.tipo_grupo || 'federal';
        const alvo = agrupado[grupo] || agrupado.federal;
        alvo.push({
          id: item.id,
          tributo: data.tributo || data.tipo || 'Debito',
          competencia: data.competencia || '',
          valor: Number(data.valor || 0),
          vencimento: data.vencimento || '',
          status: data.status || 'pendente',
          diasAtraso: Number(data.diasAtraso || data.dias_atraso || 0),
          inscricao: data.inscricao,
          dataInscricao: data.dataInscricao || data.data_inscricao,
          situacao: data.situacao,
        });
      });
      setDebitos(agrupado);
    } catch (error) {
      console.error('Erro ao consultar debitos:', error);
      setDebitos({ situacao: 'regular', federal: [], estadual: [], municipal: [], fgts: [], dividaAtiva: [] });
    } finally {
