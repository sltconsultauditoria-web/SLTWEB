...
const Fiscal = () => {
  ...
  const handleCalcularSimples = async () => {
    ...
    setResultado(null);
    try {
      const response = await api.post('/fiscal/calcular/das', simplesData);
  ...
  const handleCalcularFatorR = async () => {
    ...
    setResultado(null);
    try {
      const response = await api.post('/fiscal/calcular/fator-r', fatorRData);
      setResultado(response.data);
    } catch (error) {
      console.error('Erro:', error);
      alert('Erro ao calcular: ' + (error.response?.data?.detail || error.message));
  ...
  const handleConsultarCertidoes = async () => {
    ...
    setDadosEcac(null);
    try {
      const response = await api.get(`/certidoes/?cnpj=${cnpjEcac.replace(/\D/g, '')}`);
  ...
  const handleConsultarPendencias = async () => {
    ...
    setDadosEcac(null);
    try {
      const response = await api.get(`/debitos/?cnpj=${cnpjEcac.replace(/\D/g, '')}`);
      setDadosEcac(response.data);
    } catch (error) {
      console.error('Erro:', error);
      alert('Erro na consulta: ' + (error.response?.data?.detail || error.message));
        ...
        <TabsContent value="ecac" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Consulta de certidões e pendências fiscais persistidas no MongoDB
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                        ...
                        <div>
                          ...
                          {cert.data_validade && (
                            <p className="text-sm text-gray-500">
                              Validade: {formatarData(cert.data_validade)}
                            </p>
                          )}
                        </div>
                      </div>
