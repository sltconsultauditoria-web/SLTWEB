    ...
    for root, _, files in os.walk(FRONTEND_SRC):
        for file in files:
            if file.endswith((".js", ".jsx")):
                ...
                # Corrige imports em Layout e UI (precisa subir 2 níveis)
                if "import API from '../api.js'" in new_content and ("components/Layout" in path or "components/ui" in path):
                    new_content = new_content.replace("import API from '../api.js'", "import API from '../../api.js'")

                # Corrige imports em páginas (precisa subir 1 nível)
