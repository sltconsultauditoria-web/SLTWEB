...
router = APIRouter(prefix="/api/tipos_documentos", tags=["tipos_documentos"])
...
async def create_item(item: Tipos_documentosCreate):
    result = await db["tipos_documentos"].insert_one(item.dict())
    item_dict = item.dict()
...
async def update_item(item_id: str, item: Tipos_documentosUpdate):
    await db["tipos_documentos"].update_one({"_id": ObjectId(item_id)}, {"$set": item.dict(exclude_unset=True)})
