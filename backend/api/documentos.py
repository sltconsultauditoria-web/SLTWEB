...
async def visualizar_documento(
    ...
    try:
        documento = await service.obter_documento_por_id(documento_id)
        if not documento:
...
async def baixar_documento(
    ...
    try:
        arquivo = await service.baixar_documento(documento_id)
        if not arquivo:
            raise HTTPException(status_code=404, detail="Documento não encontrado")
        return StreamingResponse(arquivo, media_type="application/octet-stream")
    except Exception as e:
...
async def excluir_documento(
    ...
    try:
        sucesso = await service.excluir_documento(documento_id)
        if not sucesso:
            raise HTTPException(status_code=404, detail="Documento não encontrado")
    except Exception as e:
