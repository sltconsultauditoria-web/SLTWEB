...
def normalizar_certidao(certidao):
    """Garante que todos os campos esperados estejam presentes."""
    certidao.setdefault("dias_para_vencer", 0)
    certidao.setdefault("status", StatusCertidao.VALIDA)
    certidao.setdefault("created_at", datetime.utcnow())
    certidao.setdefault("updated_at", None)
    if isinstance(certidao.get("data_emissao"), str):
        certidao["data_emissao"] = date.fromisoformat(certidao["data_emissao"])
    if isinstance(certidao.get("data_validade"), str):
        certidao["data_validade"] = date.fromisoformat(certidao["data_validade"])
    return certidao
...
async def listar_certidoes(
    ...
    certidoes = [normalizar_certidao(cert) for cert in await cursor.to_list(length=por_pagina)]
...
async def obter_certidao(certidao_id: str, db=Depends(get_db)):
    ...
    return normalizar_certidao(certidao)
