...
from backend.services.fiscal_calculation_service import FiscalCalculationService
from backend.services.ecac_service import ecac_service
...
async def consultar_certidoes_ecac(cnpj: str):
    """
    Consulta certidões via e-CAC (implementação real)
    """
    try:
        # Integração real com e-CAC
...
async def consultar_pendencias_ecac(cnpj: str):
    """
    Consulta pendências fiscais via e-CAC (implementação real)
    """
    try:
        pendencias = await ecac_service.consultar_pendencias(cnpj)
        return {
            "status": "SUCESSO",
            **pendencias
        }
