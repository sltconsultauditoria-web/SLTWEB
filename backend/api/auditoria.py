...
async def listar_auditorias(
    ...
    logger.debug(f"Parâmetros recebidos: cnpj={cnpj}, empresa_id={empresa_id}, tipo={tipo}, limit={limit}")
    try:
        auditorias = await service.listar_auditorias(
            cnpj=cnpj, empresa_id=empresa_id, tipo=tipo, limit=limit
        )
        logger.debug(f"Auditorias retornadas: {auditorias}")
        return {"auditorias": auditorias, "total": len(auditorias)}
    except Exception as e:
        logger.error(f"Erro ao listar auditorias: {e}")
        raise HTTPException(status_code=500, detail=f"Erro interno ao listar auditorias: {e}")
