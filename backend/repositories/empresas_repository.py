...
class EmpresaRepository(BaseRepository):
    ...
    async def list(self):
        cursor = self.collection.find().sort("razao_social", 1)
    ...
    async def get_by_id(self, empresa_id):
        ...
        return await self.collection.find_one({"_id": empresa_id})
    ...
    async def delete(self, empresa_id):
        ...
        result = await self.collection.delete_one({"_id": empresa_id})
        return result.deleted_count > 0

    async def delete_by_cnpj(self, cnpj):
        result = await self.collection.delete_one({"cnpj": cnpj})
        return result.deleted_count > 0
