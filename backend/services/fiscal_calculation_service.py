...
# Import corrigido (caminho absoluto dentro do backend)
    ...
    async def processar_simples_nacional(
        ...
        if resultado.get("status") == "SUCESSO":
            registro = {
                ...
                "receita_bruta_12m": receita_bruta_12m,
                "receita_mensal": receita_mensal,
                "aliquota_efetiva": resultado.get("aliquota_efetiva"),
                "valor_das": resultado.get("valor_das"),
                "anexo": resultado.get("anexo"),
                "fator_r": fator_r,
                "excede_sublimite": resultado.get("excede_sublimite_estadual"),
                "detalhes": resultado,
            ...
            logger.info(f"Cálculo persistido: DAS=R$ {resultado.get('valor_das', 0):,.2f}")
    ...
    async def processar_fator_r(
        ...
        if resultado.get("status") == "SUCESSO":
            registro = {
                ...
                "folha_salarios_12m": folha_salarios_12m,
                "receita_bruta_12m": receita_bruta_12m,
                "fator_r": resultado.get("fator_r"),
                "fator_r_percentual": resultado.get("fator_r_percentual"),
                "enquadramento": resultado.get("enquadramento"),
                "detalhes": resultado,
                "created_at": datetime.utcnow()
            }
            ...
            logger.info(f"Fator R persistido: {resultado.get('fator_r_percentual', 0):.2f}%")
