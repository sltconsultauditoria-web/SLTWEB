...
class AuditoriaService:
    ...
    async def listar_auditorias(
        ...
        logger.debug(f"Filtro aplicado para listar auditorias: {filtro}")

        logger.debug("Iniciando consulta ao MongoDB para listar auditorias")
        ...
        try:
            ...
            auditorias = await cursor.to_list(length=limit)
            logger.debug(f"Dados brutos retornados: {auditorias}")
        except Exception as e:
            logger.error(f"Erro ao consultar o MongoDB: {e}")
            raise HTTPException(status_code=500, detail="Erro ao consultar o banco de dados.")

        # Adicionar log para validação de conversão
        try:
            auditorias = mongo_list_to_dict_list(auditorias)
            logger.debug(f"Dados convertidos: {auditorias}")
        except Exception as e:
            logger.error(f"Erro ao converter dados: {e}")
            raise HTTPException(status_code=500, detail="Erro ao processar os dados retornados.")
