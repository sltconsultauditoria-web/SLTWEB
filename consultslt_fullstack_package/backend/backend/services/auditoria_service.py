...
class AuditoriaService:
    ...
    async def listar_auditorias(
        ...
        logger.debug(f"Auditorias brutas retornadas do MongoDB: {auditorias}")

        # Validar e converter os dados
        try:
            auditorias = mongo_list_to_dict_list(auditorias)
            logger.debug(f"Auditorias convertidas: {auditorias}")
        except Exception as e:
            logger.error(f"Erro ao converter auditorias: {e}")
            raise HTTPException(status_code=500, detail="Erro ao processar auditorias.")
