            ...
            if file.endswith(".js"):
                ...
                try:
                    with open(path, encoding="utf-8") as f:
                        content = f.read()
                    # Captura app.use("/api/...") e router.get/post/put/delete("/...")
                    matches_use = re.findall(r'app\.use\([\'"](/api/[a-zA-Z0-9_/]+)', content)
                    matches_router = re.findall(r'router\.(get|post|put|delete)\([\'"](/api/[a-zA-Z0-9_/]+)', content)
                    rotas.extend(matches_use)
                    rotas.extend([m[1] for m in matches_router])
                except Exception:
                    pass
    return set(rotas)
