...
router = APIRouter(prefix="/api/tipos_certidoes", tags=["tipos_certidoes"])
...
async def create_item(item: Tipos_certidoesCreate):
    result = await db["tipos_certidoes"].insert_one(item.dict())
    item_dict = item.dict()
...
async def update_item(item_id: str, item: Tipos_certidoesUpdate):
    await db["tipos_certidoes"].update_one({"_id": ObjectId(item_id)}, {"$set": item.dict(exclude_unset=True)})
