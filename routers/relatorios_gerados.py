...
router = APIRouter(prefix="/api/relatorios_gerados", tags=["relatorios_gerados"])
...
async def create_item(item: Relatorios_geradosCreate):
    result = await db["relatorios_gerados"].insert_one(item.dict())
    item_dict = item.dict()
...
async def update_item(item_id: str, item: Relatorios_geradosUpdate):
    await db["relatorios_gerados"].update_one({"_id": ObjectId(item_id)}, {"$set": item.dict(exclude_unset=True)})
